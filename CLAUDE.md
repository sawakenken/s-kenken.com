...
2. **最低でも複数案を出してから着手する。**「とりあえず1案で実装」は禁止。方向性・トレードオフを並べてけんたに選ばせる。
   - **🚫 「おかしい／違和感／ダサい」系の美的フィードバックを、反射的な数値いじり（width/padding/色を1個変える）で逃げない。** それは設計・センスの問題＝`taste-skill`＋`ui-ux-pro-max` を起動して**根本から複数案**を出し、Playwright で比較する。けんたに「これこそ設計スキル使え」と言わせたら手抜き確定（2026-06-23 Contact幅で実際にやらかした）。
