...
> 🏢 **本社＝`~/Documents/kenken_Brain`（澤井健太の第二の脳）。** ここは「制作部・ポートフォリオサイト」担当の出先。全体状況・他プロジェクトとの関係は本社の `90_System/_本社ダッシュボード.md` を見る。料金・取引先・実績など仕事ナレッジは本社が正。**開発担当＝Claude Code、Codexはレビュー/助言**。
...
## 手を抜かない（最重要・けんたが繰り返し指摘している）

けんたが一番嫌うのは「楽な道で済ませる・案を出し惜しむ・勝手に切り上げる」こと。**Webサイト作業に着手する前に、必ず次をやる**：

0. **最新のWeb制作研究を1回だけ読む。** 着手前に `AI調査_引き継ぎ.md` 冒頭の「📅 最新研究インデックス」に目を通す（情報収集セッションが毎日Web制作の新ネタを反映している）。**その日の最初に1回でOK・日中は読み返さなくていい**（作業の邪魔をしない）。新しい演出・ツールが落とし込めないか一度検討してから動く。
1. **使える手段を棚卸ししてから動く。** このプロジェクトで使えるもの＝ローカルスキル `ui-ux-pro-max`(配色/UI) ・ `taste-skill`(LP/ポートフォリオの審美) ・ `copywriting`/`copy-editing`(文章) ・ `seo-audit`/`schema`/`site-architecture`(SEO/構造)、グローバルの `transitions-dev`(CSS演出)、MCP の Adobe for creativity / Canva / Playwright。**該当しそうなものは「これが使えます」と必ず挙げ、適切なら実際に使う。黙って素手で済ませない。**
2. **最低でも複数案を出してから着手する。**「とりあえず1案で実装」は禁止。方向性・トレードオフを並べてけんたに選ばせる。
   - **🚫 レイアウト・余白・配置・バランス・見た目に触れるフィードバックは、どんなに軽く・中立的でも（「気になる」「なんか」「下部」「余白でかい」等の弱い言い方も全部）＝設計問題。反射的にCSSを1プロパティ叩くのは全面禁止。** 最初の一手は必ず `taste-skill`＋`ui-ux-pro-max` エージェントを起動して構成を**根本から複数案**→Playwright比較。**padding→flex-direction→width…と部分を順番に叩くwhack-a-moleは即サボり。** ⚠ **"軽い指摘だから手で直す"が再発の正体**（2026-06-23 Contact帯で余白→下部→右半分と3連発でやらかした）。軽さで判断するな＝見た目に触れたら全部このゲートを通す。明確な機械指示（例「16pxにして」）だけ直接でOK。
3. **「これで十分」と勝手に判断して切り上げない。** けんたは"あらゆる手段の模索"を求めている。手抜き・出し惜しみは即やり直し対象。
4. **完成と言う前に実機確認**（Playwright MCP で実ブラウザ、またはプレビュー）。「できたはず」で終わらせない。
5. **Pre-Flight証跡を残す（2026-07-12新設）。** `taste-skill` には Pre-Flight Check（着手前チェックリスト）が組み込まれている＝**発動しただけで済ませず、通した項目と落ちた項目を作業報告に1ブロックで明記する**（例：`Pre-Flight: 視線動線◯ / 余白リズム◯ / タイポ階層✗→修正`）。証跡が無い「taste-skill使いました」は未使用と同じ扱い。セクション改修のbefore/afterはPlaywrightスクショを両方撮って比較してから「完成」と言う。

## 作業の進め方

- **🔴 モデル分担の型（2026-07-18 けんた確定・全セッション共通）**: Fable5期は
  **実装・修正＝opus/sonnetエージェント**（Agent tool の model 指定。設計統合や品質勝負=opus、機械的・軽量=sonnet）／
  **コードレビュー・セカンドオピニオン＝codex**（codex:codex-rescue経由・**GPT-5.6 sol を使ってよい、場合によって5.5と使い分け**・別財布）／
  **Fable5＝要件翻訳・受入検証・指揮・けんた報告のみ**（自分で実装しない。受入は指摘箇所を全部自分の目でスクショ確認）。
  マージ級の節目は「opus網羅検証＋codexレビュー」の2系統グリーンで進める。codexジョブの結果回収は `~/.claude/plugins/data/codex-inline/state/<repo>/jobs/` の json(status)/log(本文)。
  ※運用の決定は会話で終わらせず、必ずこのCLAUDE.mdかmemoryに書いてから「決定」と言う。
- ファイルは直接編集して、commit できる状態に仕上げる。「ここを手で直して」みたいな指示出しではなくこちらで書き換える。
